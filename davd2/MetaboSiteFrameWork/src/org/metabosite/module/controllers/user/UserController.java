...
public class UserController implements Serializable, Reinitialize {
	...
	public String create() {
		...
		try {
			...
			if (getEjbFacadeLocal().findById(this.user.getLogin()) != null) {
				JsfUtil.addErrorMessage(mbfe_rb.getString("Login existant"));
			} else {
				getEjbFacadeLocal().create(this.user);
	...
	public String connect() {
		...
		if (userCo != null) {
			...
			saveUserCoSession();
			Global.setFilesPathUser(userCo.getLogin());

			return Nav.ModIndex.getDestination();
		} else {
			userCo = tmp;
			return null;
		}
